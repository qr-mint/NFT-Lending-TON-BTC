import { beginDict, Address, beginCell, toNano, TupleSlice, WalletContract } from "ton";
import { sendInternalMessageWithWallet , user_principals_packed_dict} from "../tests/executor-utils";
...
export function initData() {
  ...
  return beginCell().storeAddress(masterContractAddress).storeAddress(userSC).storeDict(beginDict(256).endDict()).storeInt(new BN(0), 1).endCell();
}
...
export function initMessage() {
  return beginCell()
    .storeUint(2, 32)
    .storeUint(0, 64)
    .storeDict(user_principals_packed_dict(Address.parse("EQC9rAR21X7UK6fF_wmoto4eUYzHxevqoj1c96YJr4vBUD97")))
    .endCell();
}
